...
X = np.vstack([data_fingers, data_flowers, data_forks, data_icecreams, data_sailboats])  
...
y = np.concatenate([
    np.full(len(data_fingers), 0, dtype=object),
    np.full(len(data_flowers), 1, dtype=object),
    np.full(len(data_forks), 2, dtype=object),
    np.full(len(data_icecreams), 3, dtype=object),
    np.full(len(data_sailboats), 4, dtype=object)
])
y = y.astype(int)
...
model = tf.keras.models.Sequential([
    ...
    tf.keras.layers.Dense(11, input_shape=[784]), # input
